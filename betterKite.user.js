    ...
    if (selectedRows.length > 0) {
        calculateMargin(selection).then(margin => {
            if (selectedRows.length > 0) {
                ...
                jQ(jQ("div.positions > section.open-positions.table-wrapper > div > div > table > tfoot")[0]).append(`<tr random-att='marginsave'><td colspan='5'>Using PnL showin by Zerodha${createPnlText(pnl, maxPnl, margin)}</td><td colspan='4'>Using PnL calculated usig actualy quantity${createPnlText(realPnl, maxPnl, margin)}</td></tr>`);
